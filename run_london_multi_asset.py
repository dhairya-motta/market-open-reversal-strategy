...
def run_simulation(data_path, years_to_run, range_start_str, range_end_str, entry_start_str, entry_end_str, target_r):
    asset_name = os.path.basename(data_path).split('_')[0].upper()
    variant_name = f"{range_start_str}-{range_end_str} | Entry: {entry_start_str}-{entry_end_str} | Target: {target_r}R"
    print(f"\n=============================================")
    print(f"Loading {asset_name} dataset for {years_to_run}-Year Backtest...")
    print(f"Variant: Range {variant_name}")
    ...
    print(f"Filtered to {years_to_run} years: {start_date.date()} to {max_date.date()}")
    ...
    range_start = pd.Timestamp(range_start_str).time()
    range_end = pd.Timestamp(range_end_str).time()
    entry_start = pd.Timestamp(entry_start_str).time()
    entry_end = pd.Timestamp(entry_end_str).time()
    ...
    for row in df.itertuples():
        ...
        if row.Index.minute == 0 and row.Index.hour == 0 and row.Index.day == 1:
            if row.Index.month % 3 == 0:  # Print quarterly
                pass
        ...
        if day_of_week == 4 and time_only >= pd.Timestamp('15:50').time() and in_trade:
            ...
            events.append({'date': date_only, 'type': f'{bias}_FRIDAY_EXIT', 'pnl': pnl, 'phase': account.phase, 'status': res})
            in_trade = False
        ...
        if in_trade:
            if bias == 'LONG':
                if r_low <= trade_sl:
                    ...
                    events.append({'date': date_only, 'type': 'LONG_SL', 'pnl': -loss, 'phase': account.phase, 'status': res})
                    in_trade = False
                    ...
                    events.append({'date': date_only, 'type': 'LONG_TP', 'pnl': profit, 'phase': account.phase, 'status': res})
                    in_trade = False
                    bias = None
                    stop_trading_session = True
            elif bias == 'SHORT':
                if r_high >= trade_sl:
                    ...
                    loss = START_BAL * RISK_PER_TRADE
                    res = account.process_pnl(-loss, current_date)
                    events.append({'date': date_only, 'type': 'SHORT_SL', 'pnl': -loss, 'phase': account.phase, 'status': res})
                    in_trade = False
                    ...
                    profit = START_BAL * RISK_PER_TRADE * actual_rr
                    res = account.process_pnl(profit, current_date)
                    events.append({'date': date_only, 'type': 'SHORT_TP', 'pnl': profit, 'phase': account.phase, 'status': res})
                    in_trade = False
                    bias = None
                    stop_trading_session = True
        ...
        if time_only == range_start and not in_trade:
            sl_count = 0
            bias = None
            range_high = None
            range_low = None
            ...
            if bias == 'LONG':
                ...
                if c > f236:
                    if c <= f400:
                        ...
                        trade_tp = trade_entry + (risk * target_r)
                        entry_time = current_time
                    else:
                        ...
                        limit_order_tp = limit_order_price + (risk * target_r)
...
if __name__ == "__main__":
    grx_data = r'C:\Users\kingcuber\.gemini\antigravity-ide\scratch\grx_cleaned_2019_2024.csv'
    
    print("\n--- LONDON SESSION (DAX 2.0R) ---")
    # Variant A
    run_simulation(grx_data, 5, '01:10', '02:50', '03:00', '05:00', 2.0)
    
    # Variant B
    run_simulation(grx_data, 5, '01:10', '02:22', '02:30', '05:00', 2.0)
